...
def upload_image_to_supabase(image_bytes, filename):
    if not supabase:
        return None
    try:
        # Delete old file first to avoid upsert issues
        try:
...
def parse_result(result):
    ...
    if parsed:
        return parsed
    return {"status": "unknown", "diagnosis": str(result), "cause": "", "solution": "", "confidence": "--"}
...
def dashboard():
    ...
    if supabase:
        try:
            # Get real total count without limit
            count_resp = supabase.table("scans").select("*", count="exact").execute()
            total_count = count_resp.count or 0

            # Fetch latest 200 for display
            response = (
...
def analyze():
    try:
        ...
        if not image_url:
            ...
            print(f"WARNING: saved locally (temporary): {filename}")
        ...
        if client:
            try:
                ...
                if "429" in err or "RESOURCE_EXHAUSTED" in err or "quota" in err.lower():
                    result_json = {
                        "status": "error",
                        "diagnosis": "Gemini quota exceeded (20/day free limit)",
                        "cause": "Too many scans today. Free Gemini API allows 20 requests/day.",
                        "solution": "Wait until tomorrow OR reduce scan interval to 3 hours in Arduino code (interval = 10800000)",
                        "confidence": "--"
                    }
                else:
                    result_json = {
